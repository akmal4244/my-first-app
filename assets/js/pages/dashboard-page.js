...
async function loadSummary({ force = false } = {}) {
  ...
  try {
    ...
    showToast("Ralat dashboard", err.message || "Dashboard tidak dapat dimuatkan.", "error");
...
function renderError() {
  ...
  document.querySelector("#categoryList").innerHTML = `<p class="text-sm font-bold text-red-600">Dashboard tidak dapat dimuatkan.</p>`;
}
  ...
  document.body.innerHTML = `
    ...
    <main class="mx-auto grid w-full max-w-7xl gap-4 px-4 py-4 sm:px-6 lg:grid-cols-[18rem_minmax(0,1fr)] lg:px-8">
      ...
      <section class="grid content-start gap-5">
        <div class="brand-cover rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p class="text-xs font-extrabold uppercase tracking-widest text-blue-600">Fasa 5</p><h1 class="mt-2 text-2xl font-extrabold tracking-tight text-slate-900 sm:text-3xl">Dashboard Analitik</h1><p class="mt-3 max-w-3xl text-sm font-semibold leading-6 text-slate-500">Ringkasan risiko, status semakan dan tindakan pembetulan untuk bacaan pantas pengurusan.</p></div><button id="refreshBtn" class="rounded-full bg-blue-600 px-4 py-3 text-xs font-extrabold text-white"><i class="fa-solid fa-rotate-right mr-2"></i>Refresh</button></div></div>
...
function sidebar() {
  return `<aside class="self-start rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-20"><div class="flex items-center justify-between gap-3"><p class="text-[11px] font-extrabold uppercase tracking-widest text-slate-400">Ruang kerja</p><span id="sidebarRole" class="rounded-full bg-blue-50 px-3 py-1 text-[11px] font-extrabold uppercase tracking-wide text-blue-700">SPRAD</span></div><div class="mt-4 space-y-2">${navLinks()}</div></aside>`;
}
